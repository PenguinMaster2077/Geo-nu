...
bool GetReconInfo(Double_t &Energy, TVector3 &Position, RAT::DS::EV &ev,
...
bool SearchDelayed(RAT::DU::DSReader &dsreader,  unsigned int &iEntry, unsigned int &iEV,
Double_t &Delta_T, TVector3 &Delta_R,
Double_t &PromptEnergy, TVector3 &PromptPos, Double_t &DelayedEnergy, TVector3 &DelayedPos, 
unsigned int &DelayedEntry, unsigned int &DelayedEV, Int_t &DelayedGTID, UInt_t &DelayedNhits,
...
const UInt_t NHitsCorrected[], const ULong64_t Count50MHz[],
...
{   
    ...
    // std::string OutFile = "/home/shuaioy/projects/def-ckrauss/shuaioy/work/Geo/Ingrida/Code/Coincidence.root";
...
//Create Branch
    ...
    outtree->Branch("PromptPosY", &res.C_Prompt.C_Pos_Z, "PromptPosZ/D");
    //Delayed
    ...
    outtree->Branch("DelayedPosY", &res.C_Delayed.C_Pos_Z, "DelayedPosZ/D");
//Read File
...
//Loop
    ...
    ULong64_t Count_50MHz[2] = {99999, 99999};
    ...
    {
        if(iEntry % 100 == 0){std::cout << "已处理" << iEntry << ",还剩" << dsreader.GetEntryCount() - iEntry << std::endl;};
        ...
        //Find Prompt Event
            if(GetReconInfo(Energy[0], Position[0], ev, GTID[0], NHits[0], NHits_Cleaned[0], NHits_Corrected[0], Count_50MHz[0], recon_cali, detector_state_correct) == false){continue;};//skip the events that doesn't have reconcted info
        ...
        //Find Delayed Event
            if(SearchDelayed(dsreader, iEntry, iEv, Delta_T, Delta_R, Energy[0], Position[0],
            Energy[1], Position[1], Entry[1], EV[1], GTID[1], NHits[1], NHits_Cleaned[1], NHits_Corrected[1], Count_50MHz[1], recon_cali, detector_state_correct) == false){continue;};
        //Record Coincidence pair
            std::cout << "Find Coincidence Pair!!!Recording!!!" << std::endl;
            RecordCoincidencePair(res, Energy, Entry, EV, GTID, NHits, NHits_Cleaned, NHits_Corrected, Count_50MHz, Position[0], Position[1]);
            // res.ShowPrompt();
            // res.ShowDelayed();
...
//Get Info
    ...
    Position = fit_vertex.GetPosition();
...
RAT::DU::ReconCalibrator &recon_cali, RAT::DU::DetectorStateCorrection &detector_state_correct)
{
    ...
    for(unsigned int iDEntry = iEntry ; iDEntry < dsreader.GetEntryCount(); iDEntry++)
    {
        ...
        for(unsigned int iDEv = iEV; iDEv < delayed_entry.GetEVCount(); iDEv++)
        {
            ...
            if(!(GetReconInfo(DelayedEnergy, DelayedPos, delayed_ev, DelayedGTID, DelayedNhits, DelayedNhitsCleaned, DelayedNhitsCorrected, Delayed50MHz, recon_cali, detector_state_correct))){continue;};
...
//Delayed
    ...
    res.C_Delayed.C_Pos_X = PromptPos.X();
    res.C_Delayed.C_Pos_Y = PromptPos.Y();
    res.C_Delayed.C_Pos_Z = PromptPos.Z();
}