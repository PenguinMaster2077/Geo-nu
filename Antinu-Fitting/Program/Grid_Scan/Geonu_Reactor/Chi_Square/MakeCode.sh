...
do
	FORMATTED_INDEX=$(printf "%03d" $index)
        ...
        if [[ $index -eq 0 ]]; then
        	echo "  Scan_Geonu_Reactor(Geo_Range, Reactor_Range, Steps, FITTER_CHI_SQUARE, true);" >> $FILE
    	else
        	echo "  Scan_Geonu_Reactor(Geo_Range, Reactor_Range, Steps, FITTER_CHI_SQUARE, false);" >> $FILE
	    fi
	echo " 	return 0;" >> $FILE
	echo "}" >> $FILE
done
